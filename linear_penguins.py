...
print(f"Мера мультиколлинеарности M = D - Σd_yj = {multicollinearity_measure_new:.4f}")

#Введем привычную меру мультиколлинеарности, VIF - к-ф инфляции дисперсии - глвный метод меры мультиколлинеарности
VIF_new = 1/1-D_new
print(f"Мера мультиколлинеарности VIF new: {VIF_new:.4f}")
print("В целом исключение фактора х4 было целесообразным на первый взгляд, модель не потеряла в прогнозной силе и вырос F критерий, избавились от проблемы мультиколлинеарности факторов х3 и х4.")
...
print("\nГипотеза 2: Корреляция между факторами отсутствует")